...
def _row(document_id, nombre, case_id, source_table="enrichment_actor", es_institucional=0):
    return {
...
def test_load_rows_excludes_event_source_table_which_has_no_actor_names():
    """enrichment_event 'nombre' es la descripcion completa del
    hito, no un actor -- debe seguir excluida en la v2."""
    assert "enrichment_event" not in net.ACTOR_SOURCE_TABLES
...
def test_load_rows_against_real_warehouse_never_returns_event_descriptions():
    ...
    conn.close()
    assert len(rows) > 0
    assert all(r["source_table"] != "enrichment_event" for r in rows)
