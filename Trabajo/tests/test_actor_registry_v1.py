...
import build_actor_registry_v1 as reg  # noqa: E402
...
def test_sea_alias_all_resolve_to_same_entity_in_warehouse():
    ...
    if "actor_alias" not in tables:
        ...
        conn.close()
        pytest.skip("tabla actor_alias no existe -- correr build_actor_registry_v1.py primero")
...
def test_coverage_counts_all_three_source_tables_and_matches_real_totals():
    ...
    enrichment_institucion_v3_2, subestimando severamente el alcance real
    ...
    conn.close()
    counts = dict(rows)
    assert counts.get("enrichment_actor_v3_2", 0) > 0
    assert counts.get("enrichment_institucion_v3_2", 0) > 0
