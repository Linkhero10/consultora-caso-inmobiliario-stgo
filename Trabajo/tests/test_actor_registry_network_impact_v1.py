...
import build_actor_registry_network_impact_v1 as impact  # noqa: E402
    ...
    rows = [
        {
            ...
            "source_table": "enrichment_actor_v3_2",
...
def test_edges_deduplicate_when_aliases_share_a_conflict():
    ...
    rows_raw = net._annotate(
        [
            {"document_id": "d1", "nombre": "Contraloría", "case_id": "conflict:X", "source_table": "enrichment_actor_v3_2", "es_institucional": 0},
            {"document_id": "d2", "nombre": "Contraloría General de la República", "case_id": "conflict:X", "source_table": "enrichment_institucion_v3_2", "es_institucional": 1},
        ]
    )
    resolved = impact.resolve_identity(
        rows_raw,
...
def _connect_or_skip():
    ...
    if "actor_registry" not in tables:
        ...
        conn.close()
        pytest.skip("tabla actor_registry no existe -- correr build_actor_registry_v1.py primero")
    return conn
...
def test_contraloria_ties_with_corte_suprema_after_resolution():
    ...
    registry_rows, alias_rows = __import__("build_actor_registry_v1").build_registry()
    alias_to_entity, entity_label = impact.build_alias_map(conn)
    entity_ids = set(entity_label.keys())

    rows_raw = net._annotate(__import__("build_actor_conflict_network_v1").load_rows_conflict(conn, impact.VIEW))
