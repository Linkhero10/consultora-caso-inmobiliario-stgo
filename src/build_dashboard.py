...
<section id="tab-contexto" class="panel">
  ...
  <div class="controls">
    <label>Colorear comunas por: <select id="metricSelect">
      ...
      <option value="n_projects_verified">Proyectos con vínculo territorial verificado</option>
  ...
  <p class="note" id="projectsMentionedNote" style="display:none">"Proyectos con vínculo territorial verificado" cuenta solo menciones de proyecto vinculadas a un case_mention real con su propia comuna resuelta (case_mention_index verificado por el modelo + comuna del case_mention) -- las menciones sin ese vínculo (índice nulo, case_mention excluido, o sin comuna resuelta) se excluyen del conteo en vez de atribuirse a la comuna del documento. Ver audit/project_mention_geography_report.json para el desglose completo.</p>
...
function renderMap() {{
    ...
    geoLayer = L.geoJSON(D.territories.map(t => ({{
      ...
      onEachFeature: (f, layer) => {{
        ...
        layer.bindPopup(
          ...
          `Con respaldo de evidencia detectado: ${{t.n_conflicts_backed}}<br>Proyectos con vínculo verificado: ${{t.n_projects_verified}}<br>` +
  ...
  document.getElementById('metricSelect').addEventListener('change', e => {{
    draw(e.target.value);
    projectsMentionedNote.style.display = e.target.value === 'n_projects_verified' ? 'block' : 'none';
  }});
