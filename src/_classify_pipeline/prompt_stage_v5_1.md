...
FRONTERA DEL UNIVERSO AMPLIO: incluye desarrollo de proyectos, transformación física, regulación/permiso/uso de suelo, impactos urbanos o conflictos sobre un inmueble existente cuando ese inmueble tiene una relevancia urbana explícita. No incluyas como caso inmobiliario/urbano amplio la mera ocupación, propiedad, arriendo, desalojo o abandono de un inmueble existente si el texto no vincula el conflicto con desarrollo, transformación, regulación, uso de suelo o una consecuencia urbana. En esos casos usa `relacion_inmobiliaria_urbana=ocupacion_propiedad_sin_desarrollo`.
...
Devuelve exclusivamente el JSON definido por `classification_schema_v5_1.json`. No agregues claves, no inventes fechas, actores, instituciones, códigos INE ni resultados que el texto no contenga.
